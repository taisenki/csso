 ...
 *
 */
public class CssoClientLogoutFilter extends BaseClientFilter{
	
	private static  Logger logger = Logger.getLogger(CssoClientLogoutFilter.class.getName());
	 ...
	 */
	protected String appClientLogoutHandlerClass = "com.github.ebnew.csso.app.custom.cssoAppClientLogoutHandlerImpl";
	...
	public void doInit(FilterConfig filterConfig) throws ServletException {
		...
		//构造登录本应用的处理器对象。
		if(!BaseUtil.isEmpty(appClientLogoutHandlerClass)){
			try{
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		try{
			//本地应用已经登录，则进行登出处理。
			if(session!=null && session.getAttribute(CssoClientFilter.USER_STATE_IN_SESSION_KEY)!=null){
				if(session.getAttribute(CssoClientFilter.USER_STATE_IN_SESSION_KEY)!=null){
					//清除session中的值。
					session.setAttribute(CssoClientFilter.USER_STATE_IN_SESSION_KEY, null);
				}
