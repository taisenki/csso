...
public class AppServiceImpl extends FileSystemDao implements AppService {
	 ...
	 * 外部数据文件地址，优先级更高。
	 */
	public static final String  DEFAULT_EXTERNAL_DATA =  "D:\\workspace\\csso\\csso-core\\src\\main\\resources\\apps.js";
	 ...
	 * csso服务器本身的应用配置信息。
	 */
	private App cssoServerApp = null;
		...
		try{
			...
			if(apps!=null){
				...
				for(App app:apps){
					...
					//设置csso应用服务器。
					if(cssoServerApp==null){
						if(app.isCssoServer()){
							this.cssoServerApp = app;
						}
					}
				}
				apps = null;
	...
	@Override
	public App findCssoServerApp() {
		return this.cssoServerApp;
	}
