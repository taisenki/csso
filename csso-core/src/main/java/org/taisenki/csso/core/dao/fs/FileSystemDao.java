package org.taisenki.csso.core.dao.fs;
...
public abstract class FileSystemDao {
	...
	public String readDataFromFile(){
		try{
			...
			//优先使用外部数据文件。
			if(!BaseUtil.isEmpty(this.getExternalData())){
				try{
			...
			//若无外部文件，则使用默认的内部资源文件。
			if(inputStream==null){
				if(!BaseUtil.isEmpty(this.getClassPathData())){
					try{
						inputStream = Thread.currentThread().getContextClassLoader()
								.getResourceAsStream(this.getClassPathData());
					}catch (Exception e) {
						inputStream = null;
					}
				}
