...
public class LogoutAction {
	...
	public ModelAndView logout(HttpServletRequest request,
		...
		//调用servie统一登出所有的应用。
		this.cssoService.logout(credential, service);
		...
		if(cookies!=null && cookies.length>0){
			for(Cookie cookie:cookies){
				if(WebConstants.CSSO_SERVER_ENCRYPTED_CREDENTIAL_COOKIE_KEY.equals(cookie.getName())){
					...
					LOGGER.info("clear up the cookie "+WebConstants.CSSO_SERVER_ENCRYPTED_CREDENTIAL_COOKIE_KEY);
				}
			}
		}
		...
		if(!StringUtils.isEmpty(service)){
			...
			session.setAttribute(WebConstants.USER_STATE_IN_SESSION_KEY,null);
		}
		else{
			//返回默认的登出成功页面。
