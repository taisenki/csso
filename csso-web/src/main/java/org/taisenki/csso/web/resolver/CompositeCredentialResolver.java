	...
	public Credential resolveCredential(HttpServletRequest request){
		...
		if(credential instanceof AbstractParameter){
			...
			if(abstractParameter.getParameterValue(WebConstants.SERVICE_PARAM_NAME)==null){
				if(request.getSession().getAttribute(WebConstants.CSSO_SERVICE_KEY_IN_SESSION)!=null){
					abstractParameter.getParameters().put(WebConstants.SERVICE_PARAM_NAME, request.getSession().getAttribute(WebConstants.CSSO_SERVICE_KEY_IN_SESSION));
				}
			}
		}
		return credential;
