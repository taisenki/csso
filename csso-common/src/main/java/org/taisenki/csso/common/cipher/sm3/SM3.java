package org.taisenki.csso.common.cipher.sm3;
...
//		System.out.println(Hex.toHexString(convert(out)));
		return out;
	}
	...
	{
		return back(BaseUtil.intToBytes(num));
	}

	private static int bigEndianByteToInt(byte[] bytes)
	{
		return BaseUtil.byteToInt(back(bytes));
	}
	...
	public static byte[] padding(byte[] in, int bLen)
	{
		...
		pos += padd.length;
		byte[] tmp = back(BaseUtil.longToBytes(n));
