...
import org.taisenki.csso.common.cipher.DESCipher;
...
public class EncryCredentialManagerImpl implements EncryCredentialManager{
	...
	public EncryCredentialInfo decrypt(EncryCredential encryCredential) {
		//不为空。
		if(encryCredential!=null && !BaseUtil.isEmpty(encryCredential.getCredential())){
	...
	private EncryCredentialInfo parseEncryCredential(String credential) throws InvalidEncryCredentialException{
		...
		try{
			...
			credential = BASE64.decodeS2S(credential);
			...
			if(items.length==2){
				...
				//第1个字符串不为空
				if(!BaseUtil.isEmpty(items[0])){
					//使用base64解码为源字符串。
					byte[] data =  BASE64.decodeS(items[0]);
					//查询键值。
					CasKey CasKey = keyService.findKeyByKeyId(encryCredentialInfo.getKeyId());
					if(CasKey!=null){
						//使用密钥进行解密。
						byte[] origin = DESCipher.decrypt(data, CasKey.toSecurityKey());
						//将byte数组转换为字符串。
	...
	public String encrypt(EncryCredentialInfo encryCredentialInfo) {
		...
		if(encryCredentialInfo!=null){
			try {
				...
				//再进行BASE64编码，避免传输错误。
				return URLEncoder.encode(BASE64.encodeS2S(sb.toString()), "UTF-8");
			} catch (Exception e) {
	...
	private String encryptSensitiveInfo(EncryCredentialInfo encryCredentialInfo) throws Exception{
		...
		if(CasKey!=null){
			//查询键值。
			String key = CasKey.toSecurityKey();
			if(key!=null){
				byte[] data = DESCipher.encrypt(JSON.toJSONBytes(map), key);
				//先用BASE64编码，再用URL编码。
				return BASE64.encode2S(data);
			}
			return "";
		}
		return "";
	...
	public boolean checkEncryCredentialInfo(
		...
		if(encryCredentialInfo!=null){
			//无凭据对应的用户标识，则无效。
			if(BaseUtil.isEmpty(encryCredentialInfo.getUserId())){
				return false;
			}
